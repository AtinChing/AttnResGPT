...
Attention Residuals (AttnRes) is an architectural change to transformers from Moonshot AI ([arxiv 2603.15031](https://arxiv.org/abs/2603.15031), March 2026): instead of the standard `x = x + sublayer(x)` residual, each layer's input is a learned attention over the outputs of all previous layers.

This repo is an independent reproduction of AttnRes on small-to-medium GPT-style causal LMs, plus an extension to VLM architectures (SigLIP + Flickr30K). Preliminary results are positive at the scales tested, with the effect becoming more visible at longer context. Training runs and metrics are public on W&B: [wandb.ai/atin5551-uc-davis/attnres-next-scale](https://wandb.ai/atin5551-uc-davis/attnres-next-scale).
